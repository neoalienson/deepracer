...
class TestRewardFunction(unittest.TestCase):
  ...
  def test_speed_reward(self):
    self.ro.verbose = False
    self.assertEqual(self.ro.cal_speed_reward(self.raceline_point, 2, True), 1)  

  def test_slow_speed_reward(self):
    self.ro.verbose = False
    self.assertEqual(self.ro.cal_speed_reward(self.raceline_point, 0.8, True), 0)
    self.assertEqual(self.ro.cal_speed_reward(self.raceline_point, 1.5, True), 0.5625)
    self.assertEqual(math.ceil(self.ro.cal_speed_reward(self.raceline_point, 1.6, True)* 1000), 706)    

  def test_speed_reward_on_wheel_off_track(self):
    self.ro.verbose = False
    self.ro.ALLOW_WHEEL_OFF_TRACK = True
    self.assertEqual(self.ro.cal_speed_reward(self.raceline_point, 2.0, True), 1.0)
    self.ro.ALLOW_WHEEL_OFF_TRACK = True
    self.assertEqual(self.ro.cal_speed_reward(self.raceline_point, 2.0, False), 0.0)

  def test_over_speed_no_reward(self):
    self.ro.verbose = False
    self.ro.OVER_SPEED_REWARD = 0
    self.assertEqual(self.ro.cal_speed_reward(self.raceline_point, 2.1, True), 0.0)
    self.assertEqual(self.ro.cal_speed_reward(self.raceline_point, 3.0, True), 0.0)

  def test_over_speed_reward(self):
    self.ro.verbose = False
    self.ro.OVER_SPEED_REWARD = 0.5
    self.assertEqual(math.ceil(self.ro.cal_speed_reward(self.raceline_point, 2.1, True) * 1000), 491)
    self.ro.OVER_SPEED_REWARD = 1
    self.assertEqual(math.ceil(self.ro.cal_speed_reward(self.raceline_point, 2.1, True) * 1000), 981)
    self.assertEqual(math.ceil(self.ro.cal_speed_reward(self.raceline_point, 2.5, True) * 1000), 563)
    self.assertEqual(math.ceil(self.ro.cal_speed_reward(self.raceline_point, 2.8, True) * 1000), 130)
    self.assertEqual(math.ceil(self.ro.cal_speed_reward(self.raceline_point, 3.0, True) * 1000), 0)
